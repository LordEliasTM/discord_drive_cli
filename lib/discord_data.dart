...
class DiscordData {
  Future<void> writeDataToDiscord(Uint8List data, PartialMessage message, {bool compress = true}) async {
    ...
    final bool overMessageSizeLimit = data.length > 2000;
    ...
    if (!overMessageSizeLimit) {
      ...
      final List<AttachmentBuilder> attachments = <AttachmentBuilder>[
        AttachmentBuilder(
          data: data,
          fileName: 'discordData_${message.id}',
        )
      ];

      final Message fileMessage = await message.channel.sendMessage(MessageBuilder(
        attachments: attachments,
      ));

      await message.edit(MessageUpdateBuilder(
        content: '#${fileMessage.id}',
      ));
    }
  }
  ...
  Future<Uint8List> readDataFromDiscord(PartialMessage message) async {
    final String msg = (await message.fetch()).content;
    final bool overMessageSizeLimit = msg[0] == '#';
    ...
    if (!overMessageSizeLimit) {
      ...
      final Snowflake fileMessageId = Snowflake.parse(msg.substring(1));
      final Message fileMessage = await message.channel.messages.get(fileMessageId);
